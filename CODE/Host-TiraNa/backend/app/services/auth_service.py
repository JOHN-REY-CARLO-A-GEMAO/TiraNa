...
def authenticate_host(email: str, password: str) -> Host:
    ...
    AuthError(403) for suspended accounts, and AuthError(403) for any
    account that is not yet active (awaiting_verification, inactive).

    Only hosts with status='active' are allowed to log in and receive
    a JWT token.
    """
    host = Host.query.filter_by(email=email).first()
