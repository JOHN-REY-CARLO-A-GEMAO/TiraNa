...
const navSections = [
  ...
  },
  {
    ...
    items: [
      ...
      { to: '/admin/listings', label: 'Listings', icon: 'M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4' },
  ...
  },
  {
    ...
    items: [
      ...
      { to: '/admin/support', label: 'Support', icon: 'M18.364 5.636l-3.536 3.536m0 5.656l3.536 3.536M9.172 9.172L5.636 5.636m3.536 9.192l-3.536 3.536M21 12a9 9 0 11-18 0 9 9 0 0118 0zm-5 0a4 4 0 11-8 0 4 4 0 018 0z' },
  ...
  },
  {
    label: 'System',
    items: [
      { to: '/admin/admins', label: 'Admins', icon: 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z' },
      ...
      { to: '/admin/audit', label: 'Audit Log', icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01' },
    ],
  },
]
    ...
    <div className="min-h-screen bg-[#EEEEEE]">
      ...
      <aside className={`fixed top-0 left-0 z-50 h-full w-64 bg-[#000000] text-white transform transition-transform duration-200 overflow-y-auto ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
        <div className="p-6 border-b border-[#CB2957]">
          <h1 className="text-xl font-bold">TiraNa Admin</h1>
        </div>
        <nav className="p-4">
          {navSections.map((section) => (
            <div key={section.label} className="mb-4">
              <p className="text-xs font-semibold text-[#DDDDDD] uppercase tracking-wider px-4 mb-2">{section.label}</p>
              <div className="space-y-0.5">
                {section.items.map((item) => (
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm transition-colors ${isActive ? 'bg-[#CB2957] text-white' : 'text-[#DDDDDD] hover:bg-[#DDDDDD]/10'}`
                    }
                  >
      ...
      <div className="lg:ml-64">
        <header className="bg-[#EEEEEE] shadow-sm border-b border-[#DDDDDD]">
          <div className="flex items-center justify-between px-6 py-4">
            ...
            <div className="flex items-center gap-4 ml-auto">
              <span className="text-sm text-[#000000]">{admin?.username}</span>
              <button onClick={handleLogout} className="px-4 py-2 text-sm bg-[#CB2957] hover:bg-[#CB2957]/80 text-white rounded-lg transition-colors">
                Logout
              </button>
            </div>
          </div>
